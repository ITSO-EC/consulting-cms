...
<template>
  <div :key="$route.fullPath">
    ...
    <main v-bind="$attrs" class="relative flex min-h-[60vh] z-10 overflow-x-hidden" >
      <ConsultingSidebar 
      
      :class="`animate__animated animate__fadeInLeft ${firstVisit? 'animate__delay-2s animate__slower' : '' }`" 
      v-if="$route.params.pageid" /> 
      <div class="w-full lg:w-4/5 animate__animated animate__fadeIn" :class="`${firstVisit ? 'animate__delay-4s': ''}`"  >
